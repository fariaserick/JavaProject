...
public class NumberToWord {
	public static String numberToWords(int number) {
		...
		List<String> listString = new ArrayList<String>(Arrays.asList());
		int num = reverse(number);
		int digits = getDigitCount(number);
		while(digits > 0){
			int digit = num % 10;
			listString.add(numWords[digit]);
			num /= 10;
			digits--;
		}
	...
	public static int reverse(int number) {
		String stringNum = String.valueOf(Math.abs(number));
		String sb = new StringBuilder(stringNum).reverse().toString();
		int in = Integer.parseInt(sb);
		if(number < 0) in *= -1;
		return in;
	}
